let user_det=document.getElementById("f");

const retent=()=> {
    let ent= localStorage.getItem("f1");
    if(ent){
        ent=JSON.parse(ent);
    }
    else{
...
const dispent=()=> {
    ...
    let details= document.getElementById("f1");
...
userDet.push(entry);
localStorage.setItem("f1",JSON.stringify(userDet));
dispent();
}
